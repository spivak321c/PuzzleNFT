...
IGNORE_DIRS = {".git", "node_modules", "target", "__pycache__", 
               ".idea", ".vscode", "venv", ".env", "dist", "build",".anchor","test-ledger"}
IGNORE_FILES = {".DS_Store", "Thumbs.db", ".gitignore",".prettierignore","cargo.bak","Cargo.lock","tsconfig.json","yarn.lock","parse.py","setup.sh","Makefile","start-validator.sh","README.md"}
...
def generate_tree(root: Path, prefix="", depth=0, max_depth=5):
    ...
    for i, entry in enumerate(entries):
        ...
        if entry.is_dir():
            ...
            result.extend(generate_tree(entry, prefix + extension, depth + 1, max_depth))
    return result
...
def parse_codebase(root_folder: str, output_file: str = OUTPUT_FILE):
    ...
    try:
        ...
        for file_path in root.rglob("*"):
            if any(part in IGNORE_DIRS or part in IGNORE_FILES for part in file_path.parts):
                continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Export codebase to Markdown with <xaiArtifact> tags")
    parser.add_argument("folder", help="Path to the codebase folder")
    parser.add_argument("-o", "--output", default=OUTPUT_FILE, help="Output markdown file name")
